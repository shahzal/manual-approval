...
[![ci](https://github.com/shahzal/manual-approval/actions/workflows/ci.yaml/badge.svg)](https://github.com/shahzal/manual-approval/actions/workflows/ci.yaml)
...
```yaml
steps:
  - uses: shahzal/manual-approval@v1
    with:
      secret: ${{ github.TOKEN }}
...
```dockerfile
# LABEL org.opencontainers.image.source https://github.com/shahzal/manual-approval
```

Build the image:

```
$ VERSION=1.7.1-rc.1 make IMAGE_REPO=ghcr.io/shahzal/manual-approval-test build
```
...
```
$ VERSION=1.7.1-rc.1 make IMAGE_REPO=ghcr.io/shahzal/manual-approval-test push
```

To test out the image you will need to modify `action.yaml` so that it points to your new image that you're testing:

```yaml
  image: docker://ghcr.io/shahzal/manual-approval-test:1.7.0-rc.1
```
